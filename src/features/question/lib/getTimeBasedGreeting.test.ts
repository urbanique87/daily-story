// constants
import { TIME_BASED_GREETINGS, TIME_RANGES } from "@/shared/constants/greetings"
// lib
import { getGreetingForHour, getTimeBasedGreeting } from "@/features/question"
...
describe("인사말 관련 함수 테스트", () => {
  describe("getGreetingForHour (시간별 인사말 반환 함수)", () => {
    it.each([
      {
        hours: 3,
        expected: TIME_BASED_GREETINGS.EVENING,
        description: "새벽 3시",
      },
      {
        hours: 7,
        expected: TIME_BASED_GREETINGS.MORNING,
        description: "아침 7시",
      },
      {
        hours: 14,
        expected: TIME_BASED_GREETINGS.AFTERNOON,
        description: "오후 2시",
      },
      {
        hours: 19,
      ...
      {
        hours: 22,
        expected: TIME_BASED_GREETINGS.NIGHT,
        description: "밤 10시",
      },
    ])(
      "$description에는 $expected 인사말을 반환해야 한다",
      ({ hours, expected }) => {
  ...
  describe("getTimeBasedGreeting (현재 시간 기준 인사말 반환 함수)", () => {
    ...
    it.each([
      ...
      "$description 시점에 정확한 인사말로 전환되어야 한다",
      ({ hours, expected }) => {
        mockDate(hours)
